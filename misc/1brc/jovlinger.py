...
class SerialPool:
    """Just serial Pool, like multiprocessing.Pool but simpler, also subset of methods"""
...
def seek_next_line(f) -> int:
    ...
    """
    prt(f"seek next line {f.tell()}")
    x = f.readline()
    prt(f"seek next line '{x}' -> {f.tell()}")
    return f.tell()
...
def out(n=-1, *, cnts, tots, mins, maxs):
    ...
    for name in names[:n]:
        print(f"{name} -> tots {tots[name]} / cnts {cnts[name]}, min: {mins[name]} max: {maxs[name]}")
...
def dochunk( tup) -> "tuple[cnts,tots,mins,maxs]":
    ...
    while lo < hi:
        ...
        try:
            ...
            print(f"line: '{line}' at {lo} < {hi} (tell: {f.tell()})")
            break
