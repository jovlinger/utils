...
def process_file(source_file: Path, source_root: Path, target_dir: Path,
                 processed_hashes: Set[str], used_paths: Set[str],
                 content_to_suffix: Dict[str, str],
                 verbose: bool = False, dryrun: bool = False,
...
def _process_single_file(source_file: Path, source_root: Path, target_dir: Path,
                         processed_hashes: Set[str], used_paths: Set[str],
                         content_to_suffix: Dict[str, str],
                         verbose: bool = False, dryrun: bool = False,
    ...
    """
    try:
        file_hash = process_file(source_file, source_root, target_dir,
                                 processed_hashes, used_paths, content_to_suffix,
                                 verbose, dryrun, remove_source)
        if verbose:
            relative_path = source_file.relative_to(source_root)
            files_dir = target_dir / "files"
            symlink_path = files_dir / relative_path
...
def deduplicate_sources(sources: List[Path], target_dir: Path,
                        verbose: bool = False, dryrun: bool = False,
    ...
    for source in sources:
        ...
        if source.is_file():
            if _process_single_file(source, source.parent, target_dir,
                                    processed_hashes, used_paths, content_to_suffix,
                                    verbose, dryrun, remove_source):
                files_processed += 1
        elif source.is_dir():
            ...
            for root, dirs, files in os.walk(source):
                ...
                for file_name in files:
                    ...
                    if _process_single_file(source_file, source_parent, target_dir,
                                            processed_hashes, used_paths, content_to_suffix,
                                            verbose, dryrun, remove_source):
                        files_processed += 1
...
def deduplicate_directory(source_dir: Path, target_dir: Path,
                          verbose: bool = False, dryrun: bool = False) -> Dict[str, int]:
    """Legacy wrapper for backward compatibility."""
    return deduplicate_sources([source_dir], target_dir, verbose, dryrun)


def fix_symlinks(target_dir: Path, verbose: bool = False, dryrun: bool = False) -> Dict[str, int]:
    """
    ...
    for path in sorted(files_dir.rglob("*")):
        ...
        if not data_file.exists():
            print(f"Warning: data file missing: {data_file} (from {path})", file=sys.stderr)
            broken += 1
            continue
...
def main():
    ...
    if args.fix:
        ...
        if len(args.positional) < 2:
            parser.error("Dedup mode requires at least two arguments: <source> [source...] <target_dir>")
        
        sources = args.positional[:-1]
        target_dir = args.positional[-1]
        
        try:
            stats = deduplicate_sources(sources, target_dir, args.verbose, args.dryrun, args.remove_source)
