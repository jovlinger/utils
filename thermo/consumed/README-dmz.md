...
**Install script (Docker → raw):** Use `install/export_rootfs.sh` to export the DMZ image to a rootfs tarball, then `install/run_raw.sh` to run it via bwrap (no Docker daemon). See **`DEPLOY-PI1B.md`** for step-by-step deployment on Pi 1B.
...
The Dockerfile uses an Alpine minimal base; the image can be exported to a rootfs and run via bwrap (see install scripts above). Alternative: create a folder app_root.
