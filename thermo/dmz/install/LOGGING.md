...
All logs live on **tmpfs** (`/tmp`). Nothing is written to the SD card at runtime except the one-time copy of the boot log to `debug/` before unmount.
...
- The app is started via **run-with-stdout-logged.py** (in the container image). It runs `python app.py` and appends its stdout/stderr to `/tmp/dmz.log`.
...
- **Mechanism:** `install/run_raw.sh` (launcher) invokes `python run-with-stdout-logged.py /tmp/dmz.log 1048576 2097152 sh ./run.sh`.
...
| App   | `/tmp/dmz.log`   | 1 MB file → rotate; 2 MB rotated total cap | run-with-stdout-logged.py |
