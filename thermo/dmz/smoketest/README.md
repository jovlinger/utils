...
Build the image (after **`stage-docker-import.sh`** copies **`../../../extdeps/run-with-stdout-logged.py`** into **`.docker-import/`**), run a container like **`make runlocal`**, then **`pytest test_smoke.py`** from the host venv against the live API.
