...
echo "SD forensics (overwrite debug/*): /root/dmz-forensics.sh" >> "$APKOVL_DIR/root/README"
cp "$DMZ_ROOT/install/root-network-sshd.sh" "$APKOVL_DIR/root/network-and-sshd.sh"
chmod +x "$APKOVL_DIR/root/network-and-sshd.sh"
cp "$DMZ_ROOT/install/dmz-forensics.sh" "$APKOVL_DIR/root/dmz-forensics.sh"
chmod +x "$APKOVL_DIR/root/dmz-forensics.sh"
