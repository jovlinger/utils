...
### Install Script: Docker Image → Raw Rootfs

To build the DMZ Docker image and run it **without the Docker daemon** (bwrap isolation, read-only storage), use:

- **`install/export_rootfs.sh`** – exports the image to a rootfs tarball
- **`install/run_raw.sh`** – runs the extracted rootfs via bwrap

See **`DEPLOY-PI1B.md`** for the exact steps to get the image running on Pi 1B.
