"""Midea classic 48-bit AC protocol encoder.
...
The remote repeats the main frame; some remotes also emit a model-specific
secondary frame, so the raw captures remain the source of truth while this
encoder covers the common state packet.
"""
...
def dumps(state: State) -> str:
    ...
    frame = _with_complements(_state_data_bytes(state))
