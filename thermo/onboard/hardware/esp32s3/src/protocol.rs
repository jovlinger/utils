...
pub fn build_sensor_post_body<const N: usize>(
    ...
    write_log_lines(&mut body, log_lines)?;
    write_deployment_object(&mut body, config)?;
    Ok(body)
}

pub fn build_logs_body<const N: usize>(
    log_lines: &[&str],
    capacity: usize,
    len: usize,
) -> Result<String<N>, core::fmt::Error> {
    ...
    write!(
        body,
        "{{\"log_storage\":{{\"path\":null,\"type\":\"memory\",\"capacity\":{},\"newest_limit\":{},\"len\":{}}},\"lines\":[",
        capacity,
        log_lines.len(),
        len
    )?;
...
fn write_log_lines<const N: usize>(
    body: &mut String<N>,
    log_lines: &[&str],
...
fn write_deployment_object<const N: usize>(
    ...
    write!(body, ",")?;
    write_json_string_field(body, "report_behavior", config.report_behavior)?;
    write!(body, ",")?;
    write_json_string_field(body, "sensor_driver", config.sensor_driver)?;
    write!(body, ",")?;
    write_json_string_field(body, "ir_transport", config.ir_transport)?;
    write!(body, ",")?;
    write_json_string_field(body, "ir_device", config.ir_device)?;
    write!(body, ",")?;
    write_json_string_field(body, "ir_protocol", config.ir_protocol)?;
    write!(body, ",")?;
    write_json_string_field(body, "backend", config.deploy_backend)?;
    write!(body, ",")?;
    write_json_string_field(body, "status_led_driver", config.status_led_driver)?;
...
pub(crate) fn write_json_string_field<const N: usize>(
    body: &mut String<N>,
    key: &str,
    value: &str,
) -> Result<(), core::fmt::Error> {
...
pub(crate) fn write_json_string<const N: usize>(
    out: &mut String<N>,
    value: &str,
) -> Result<(), core::fmt::Error> {
...
mod tests {
    ...
    #[test]
    fn builds_esp32s3_sensor_post_body() {
        ...
        let body =
            build_sensor_post_body::<1024>(&config, reading, None, &[], Some("192.168.1.55"))
                .unwrap();
    ...
    #[test]
    fn parses_dmz_command_and_time() {
        let body = "{\"command\":{\"created_dt\":\"2026-06-14T21:00:01.000000\",\"note\":\"brace } in string\"},\"server_time_utc\":\"2026-05-27T04:25:00.123456Z\"}";

        assert!(command_is_new("", extract_command_created_dt(body)));
        assert_eq!(
            extract_command_json(body),
            Some("{\"created_dt\":\"2026-06-14T21:00:01.000000\",\"note\":\"brace } in string\"}")
        );
        assert_eq!(parse_server_time_utc_epoch(body), Some(1_779_855_900));
    }

    #[test]
    fn builds_newest_first_logs_body() {
        let body = build_logs_body::<256>(&["poll ok", "startup"], 64, 2).unwrap();

        assert!(body.contains("\"type\":\"memory\""));
        assert!(body.contains("\"capacity\":64"));
        assert!(body.contains("\"lines\":[\"poll ok\",\"startup\"]"));
    }
}
