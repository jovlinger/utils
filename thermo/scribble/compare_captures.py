...
for fi, fr in enumerate(frames):
    raw = fr["raw"]
    print(
        f"--- Frame {fi}: {fr['file']} rec={fr['record']} {fr['label']} desc={fr['desc']!r} ts={fr['ts']} ---"
    )
    ...
    print(
        f"    byte7={raw[7]:02x} chk={sum(raw[:7])&0xff:02x} ok={dr.checksum_ok(raw[:8])}"
    )
    # F2 bytes 8-15
    print(f"  F2 [8:16]: {' '.join(f'{b:02x}' for b in raw[8:16])}")
    print(f"    byte12={raw[12]:02x} (expect 42)")
    print(
        f"    byte15={raw[15]:02x} chk={sum(raw[8:15])&0xff:02x} ok={dr.checksum_ok(raw[8:16])}"
    )
    # F3 bytes 16-26
    f3 = raw[16:27]
    ...
    if len(f3) >= 11:
        ...
        print(
            f"    byte5={b5:02x} -> mode={mode_s} power={'ON' if power_bit else 'OFF'} timer_on={timer_on} timer_off={timer_off} always1={always1}"
        )
        print(f"    byte6={temp_raw:02x} -> temp={temp_c}C")
        print(
            f"    byte8={fan_byte:02x} -> fan=0x{fan_nib:x} swing={'on' if swing_nib==0xF else 'off'}"
        )
        if len(f3) > 0xA - 16 + 16:
            # timer bytes relative to F3: offsets 0xa, 0xb, 0xc within F3 = indices 10, 11, 12 but F3 is only 11 bytes...
            pass
    print(
        f"    byte10={f3[10]:02x} chk={sum(f3[:10])&0xff:02x} ok={dr.checksum_ok(f3)}"
    )
    print()
...
for fi, fr in enumerate(frames):
    ...
    if current:
        sub_frames_pairs.append(current)
    print(
        f"--- Frame {fi}: split into {len(sub_frames_pairs)} sub-frame(s) at gaps ---"
    )
    for si, sf_pairs in enumerate(sub_frames_pairs):
        ...
        if raw_sf:
            ...
            print(
                f"    chk: sum={sum(raw_sf[:-1])&0xff:02x} last={raw_sf[-1]:02x} ok={dr.checksum_ok(raw_sf)}"
            )
            if len(raw_sf) >= 5:
                print(
                    f"    header: {' '.join(f'{b:02x}' for b in raw_sf[:4])}  id_byte={raw_sf[4]:02x}"
                )
    print()
