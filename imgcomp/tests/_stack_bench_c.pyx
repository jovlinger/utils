...
cdef int64_t ack_c(int64_t m, int64_t n) except *:
    """Recursive Ackermann function; returns -1 only on Python exception (``except *``)."""
    if m == 0:
        return n + 1
    if n == 0:
        return ack_c(m - 1, 1)
...
cdef void _op_mandel_pixel() except *:
...
cdef void _op_mandel_sum() except *:
