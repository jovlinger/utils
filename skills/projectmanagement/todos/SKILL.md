...
| `todo.py search <term>...` | implemented | Vector + lexical ticket search over one or more terms, google-style: each term is embedded and matched independently and the per-term scores add. A term is the unit of embedding -- quote a phrase (`todo search "bh 791"`) to match it whole; unquoted words (`todo search bh 791`) match individually. `-n` limit; `--embedder` comma list (default all non-hidden), `--dry-run`, `--tag` comma list (keep only todos whose `Tags` intersect it) |
...
| `Tags` | list of strings (optional) | Explicit user-set labels, persisted on the todo, kept sorted and deduped. Edited with `set --tag/--untag` (repeatable); the field is dropped when the last tag is removed. Filter searches with `search --tag=a,b` (keeps todos whose `Tags` intersect the list). Distinct from the `tag` command's ephemeral zero-shot semantic tags, which are computed at read-time and never stored. |
...
`ActualSummary` and `Tags` are optional and omitted when unused.
