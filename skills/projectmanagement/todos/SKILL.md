...
| `todo.py search <query>` | implemented | Vector + lexical ticket search (-n limit); `--embedder` comma list (default all non-hidden), `--dry-run`, `--tag` comma list (keep only todos whose `Tags` intersect it) |
