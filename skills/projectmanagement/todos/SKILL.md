...
| Embeddings | in the ticket JSON; sqlite backend also mirrors a derived embeddings index | No embedder is currently `cheap`, so a write stamps nothing and `search` backfills every vector lazily. `read` merges every embedder found in the index into its output regardless of which path wrote it, elided to its first two elements |
...
**Automatic tags are dormant (2026-08-04).** Zero-shot auto-tagging shipped
running on the `hash` embedder -- bag-of-words md5 hashing, i.e. purely lexical.
Measured on the real corpus, its tags were collision noise rather than topics:
the spec's motivating case (tag a document about testing with "testing" when the
word never appears) scored exactly 0.0. `hash` has therefore been REMOVED
outright, which leaves no `cheap` embedder, so `_recompute_auto_tags` returns 0
and `doctor` adds no tags. Consequences:

- Nothing embeds on write; `search` backfills vectors lazily (`apple`).
- `apple` is the only production embedder. Tests mock its Swift sidecar with a
  bag-of-words fake (`fake_nlce.py`) so vector paths stay hermetic off macOS.
- Auto-tagging **re-arms by itself** once a cheap semantic backend is registered.
  That successor is ticket `91e28fd0` (domain-tuned importance pipeline), which
  should also settle the two known defects before auto tags are trusted again:
  hubness correction (raw cosine picks the wrong tag even with a semantic
  embedder -- every candidate sits at 0.64-0.78) and word-level rather than
  sentence-level candidates (mined candidates currently run ~91 chars median, so
  "tags" are sentence fragments).
- Zero-norm vectors are a silent failure mode: `_is_vector` accepts an all-zero
  vector as present and `cosine_similarity` returns 0.0 for it instead of
  raising, so the trust-existing backfill never repairs one. 77% of stored
  `apple` vectors were all-zero when this was measured.
