...
7. Record the real code-landing (`work-item-add` + `work-item-done` so the last
   work item's sha == branch HEAD, invariant #6), then `todo.py set self --state done
   --actual-summary=...`.
...
- **Cross-subtodo communication (note-to-parent):** a subtodo that COMPLETES but has
  ...
  `userneeded`. It finishes and appends the concern to its `set self --state done
...
todo.py set self --state working
...
todo.py set self --state done --actual-summary="..."
```
