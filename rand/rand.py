...
class Inputs:
    ...
    def line(self):
        ...
        while self.files:
            ...
            line = self.files[self.n].readline() # '' is EOF, '/n' terminates all read lines
            if line: return line
    ...
    while True:
        ...
        if line:
            ...
            if random.random() <= (1.0/n) or buf.cap == 0:
                ...
                continue
        if buf.empty(): break
        if not buf.empty(): outfile.write(buf.some())
        if line: buf.add(line)

if __name__ == "__main__":
    ap = argparse.ArgumentParser("Randomize lines from files, or stdin.")
    ap.add_argument("-b", "--buffer", help="buffer capacity, defaults to 1000", type=int, default=1000)
    ap.add_argument("files", help="file to randomize lines from", metavar="FILE", nargs="*")
    args = ap.parse_args()
